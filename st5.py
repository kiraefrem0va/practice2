import sys
import json
import urllib.request
from pathlib import Path
import subprocess

def parse_args():
    args = {
        "package_name": None,
        "repository_url": None,
        "test_repo_mode": None,
        "package_version": None,
        "output_file": None,
        "reverse_mode": None,
        "visualize": None
    }

    argv = sys.argv[1:]
    i = 0
    while i < len(argv):
        arg = argv[i]
        if arg == "--package-name":
            i += 1
            args["package_name"] = argv[i]
        elif arg == "--repository-url":
            i += 1
            args["repository_url"] = argv[i]
        elif arg == "--test-repo-mode":
            i += 1
            args["test_repo_mode"] = argv[i]
        elif arg == "--package-version":
            i += 1
            args["package_version"] = argv[i]
        elif arg == "--output-file":
            i += 1
            args["output_file"] = argv[i]
        elif arg == "--reverse-mode":
            i += 1
            val = argv[i].lower()
            if val not in ("true","false"):
                raise ValueError("--reverse-mode должно быть true или false")
            args["reverse_mode"] = val == "true"
        elif arg == "--visualize":
            i += 1
            val = argv[i].lower()
            if val not in ("true","false"):
                raise ValueError("--visualize должно быть true или false")
            args["visualize"] = val == "true"
        else:
            raise ValueError(f"Неизвестный аргумент: {arg}")
        i += 1
        
    missing = [k for k, v in args.items() if v is None]
    if missing:
        raise ValueError(f"Отсутствуют обязательные параметры: {', '.join(missing)}")

    return args

def check_repository(repo, mode):
    if mode == "remote" and not repo.startswith(("http://","https://")):
        raise ValueError("Для remote требуется URL")
    elif mode == "local":
        path = Path(repo)
        if not path.exists():
            raise FileNotFoundError(f"Файл '{repo}' не найден")

def check_package_version(version):
    if not any(c.isdigit() for c in version):
        raise ValueError("Версия пакета должна содержать цифры")

def check_output_file(filename):
    if not filename.endswith((".tar.gz",".whl",".svg")):
        raise ValueError("Имя выходного файла должно оканчиваться на .tar.gz, .whl или .svg")

def load_local_graph(path):
    graph = {}
    with open(path,"r") as f:
        for line in f:
            line = line.strip()
            if not line or ":" not in line:
                continue
            package,deps = line.split(":",1)
            deps_list = deps.split() if deps.strip() else []
            graph[package.strip()] = deps_list
    return graph

def load_remote_graph(package_name, version):
    url = f"https://pypi.org/pypi/{package_name}/{version}/json"
    print(f"Получаем информацию о пакете {package_name} по {url}")
    with urllib.request.urlopen(url) as response:
        data = json.load(response)
    requires = data["info"].get("requires_dist")
    if not requires:
        return {package_name: []}
    deps = []
    for dep in requires:
        dep_name = dep.split(";")[0].split()[0]
        deps.append(dep_name)
    return {package_name: deps}

def get_transitive_dependencies(graph, start):
    visited = set()
    stack = [start]
    result = set()
    while stack:
        node = stack.pop()
        if node in visited:
            continue
        visited.add(node)
        for dep in graph.get(node, []):
            if dep not in result:
                result.add(dep)
                stack.append(dep)
    return result

def get_reverse_dependencies(graph, target):
    reverse_graph = {}
    for pkg,deps in graph.items():
        for dep in deps:
            reverse_graph.setdefault(dep, []).append(pkg)
    return get_transitive_dependencies(reverse_graph, target)

def generate_mermaid(graph):
    lines = ["graph TD"]
    for pkg,deps in graph.items():
        for dep in deps:
            lines.append(f"  {pkg} --> {dep}")
    return "\n".join(lines)

def save_mermaid_file(filename, mermaid_text):
    with open(filename,"w") as f:
        f.write(mermaid_text)
    print(f"Mermaid файл сохранен: {filename}")

def render_svg_from_mermaid(filename_mmd, filename_svg):
    try:
        subprocess.run(["mmdc","-i",filename_mmd,"-o",filename_svg],check=True)
        print(f"SVG файл с графом сохранен: {filename_svg}")
    except Exception as e:
        print(f"Не удалось сгенерировать SVG: {e}")

def main():
    try:
        args = parse_args()
        check_repository(args["repository_url"], args["test_repo_mode"])
        check_package_version(args["package_version"])
        check_output_file(args["output_file"])

        if args["test_repo_mode"]=="local":
            graph = load_local_graph(args["repository_url"])
        else:
            graph = load_remote_graph(args["package_name"], args["package_version"])

        print("\nГраф зависимостей:")
        for pkg,deps in graph.items():
            print(f"{pkg}: {deps}")

        transitive = get_transitive_dependencies(graph,args["package_name"])
        print(f"\nТранзитивные зависимости пакета {args['package_name']}:")
        if transitive:
            for dep in sorted(transitive):
                print(f" - {dep}")
        else:
            print(" (нет зависимостей)")

        if args["reverse_mode"]:
            reverse = get_reverse_dependencies(graph,args["package_name"])
            print(f"\nОбратные зависимости для пакета {args['package_name']}:")
            if reverse:
                for dep in sorted(reverse):
                    print(f" - {dep}")
            else:
                print(" (нет обратных зависимостей)")

        if args["visualize"]:
            mermaid_text = generate_mermaid(graph)
            mmd_file = args["output_file"].replace(".svg",".mmd")
            save_mermaid_file(mmd_file,mermaid_text)
            render_svg_from_mermaid(mmd_file,args["output_file"])

    except Exception as e:
        print(e,file=sys.stderr)
        sys.exit(1)

if __name__=="__main__":
    main()